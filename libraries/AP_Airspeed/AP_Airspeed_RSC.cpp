/*
  Description: driver for Honeywell RSC airspeed
  Author: liang.tang
  Date: 2019/09/017
*/
...
#define PRESSURE_RATIO                         249.08798f

#define RSC_DRDY_PIN 94
...
bool AP_Airspeed_RSC::init()
{
    ...
    // get eeprom device
    _eeprom_dev = hal.spi->get_device("rscAirSpeedEe");
    if (!_eeprom_dev) {
        return false;
    }

    // get adc device
    _adc_dev = hal.spi->get_device("rscAirSpeedAdc");
    if (!_adc_dev) {
        return false;
    }
    hal.console->printf("%s %d\n", __func__, __LINE__);
    ...
    _get_pressure_minimum();

    // get initial ADC values
    _get_initial_adc_values();
...
void AP_Airspeed_RSC::_timer()
{
    ...
    if (_type == TEMPERATURE) {
        ...
        _pressure_sum += _calculate_pressure() * PRESSURE_RATIO;
        _press_count++;
...
void AP_Airspeed_RSC::_get_pressure_range()
{
    uint8_t buf[RSC_PRESSURE_RANGE_LEN];
    _eeprom_read(RSC_PRESSURE_RANGE_LSB, RSC_PRESSURE_RANGE_LEN, buf);
    // convert byte array to float (buf[0] is LSB)
    memcpy(&_pressure_range, &buf, sizeof(_pressure_range));
}

void AP_Airspeed_RSC::_get_pressure_minimum()
{
    uint8_t buf[RSC_PRESSURE_MINIMUM_LEN];
    _eeprom_read(RSC_PRESSURE_MINIMUM_LSB, RSC_PRESSURE_MINIMUM_LEN, buf);
    // convert byte array to float (buf[0] is LSB)
    memcpy(&_pressure_minimum, &buf, sizeof(_pressure_minimum));
}

void AP_Airspeed_RSC::_get_initial_adc_values()
{
    ...
    for (uint8_t i = 0; i < RSC_COEFF_T_ROW_NO; i++) {
        ...
        for (uint8_t j = 0; j < RSC_COEFF_T_COL_NO; j++) {
            temp = l_coeffs[j * 4 + 0] |
                   ...
                   (l_coeffs[j * 4 + 3] << 24);
            matrix = (float*)&temp;
            _coeff_matrix[i][j] = *matrix;
        }
    }
}

void AP_Airspeed_RSC::_adc_configure()
{
    uint8_t command[5];
    uint8_t reg = 0;
    uint8_t num_bytes = 4;
...
void AP_Airspeed_RSC::_adc_convert(READING_T type)
{
    uint8_t command[2] = {0};
    // WREG byte
