...
CompStatus_Enum Compensate_Pressure_Init(unsigned char *u8EEPROM_ptr)
{
    ...
    printf("EEPROM_MAP.f32Pressure_Range is %f ++++++++++\n", EEPROM_MAP.f32Pressure_Range);
    ...
    printf("EEPROM_MAP.f32PressureOffset is %f ++++++++++\n", EEPROM_MAP.f32PressureOffset);
    ...
    printf("EEPROM_MAP.u8PressureReference is %d ++++++++++\n", EEPROM_MAP.u8PressureReference);
    ...
    for(u8i = 0u, u16k = 0u; u8i < 4u; u8i++, u16k += 2u)
    {
        ...
        printf("EEPROM_MAP.au16ADC_Config[u8i] is %d ++++++++++\n", EEPROM_MAP.au16ADC_Config[u8i]);
    }
    ...
    for(u8i = 0u, u16k = 0u; u8i < DEGREE_POLYNOMIAL; u8i++, u16k += 4u)
    {
        ...
        printf("EEPROM_MAP.af32Coeff1[u8i] is %f ++++++++++\n", EEPROM_MAP.af32Coeff1[u8i]);
    }
    ...
    for(u8i = 0u, u16k = 0u; u8i < DEGREE_POLYNOMIAL; u8i++, u16k += 4u)
    {
        ...
        printf("EEPROM_MAP.af32Coeff2[u8i] is %f ++++++++++\n", EEPROM_MAP.af32Coeff2[u8i]);
    }
    ...
    for(u8i = 0u, u16k = 0u; u8i < DEGREE_POLYNOMIAL; u8i++, u16k += 4u)
    {
        ...
        printf("EEPROM_MAP.af32Coeff3[u8i] is %f ++++++++++\n", EEPROM_MAP.af32Coeff3[u8i]);
    }
...
CompReturn_Struct Compensate_Pressure(unsigned long int u32PressureInput,unsigned long int u32Temperature)
{
        ...
        if(u32PressureInput < 16777216u)
        {
            ...
            if(u32Temperature < 16384u)
            {
                ...
                else
                {
                    ...
                    /*! Indicate the Output Pressure is invalid */
                    strCompReturn.CompStatus = PRESSURE_INVALID2;
                }
    ...
    if((runCnt++ % 200) == 1)
    {
        printf("in Compensate_Pressure function: %lu, %lu, %d <========================== \n", u32PressureInput, u32Temperature, strCompReturn.CompStatus);
        printf("in Compensate res is % d < ============= \n", strCompReturn.CompStatus);
    }
